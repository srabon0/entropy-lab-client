...
const Myprofile = () => {
  ...
  const { isLoading, error2, data:currentUser, refetch } = useQuery(['user',user], () =>
     fetch(url).then(res =>
       res.json()
     )
   )
    ...
    fetch(url, {
      ...
      .then((result) => {
        ...
        if (result.success) {
          ...
          const urlToUpdate = `http://localhost:5000/update/${currentUser.email}`;
          fetch(urlToUpdate, {
            method: "PUT",
            headers: {
  ...
  if (error || error2) {
    ...
    return (
        ...
        <div class="max-w-md overflow-hidden bg-white rounded-lg shadow-lg dark:bg-gray-800">
            <img class="object-cover object-center w-full h-48" src={ currentUser.img || "https://images.unsplash.com/photo-1517841905240-472988babdf9?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=334&q=80"} alt="avatar" />
            {
              !currentUser?.img &&<p className='text-white'> update ur image</p>
            }
         

            <div class="px-6 py-4">
                <h1 class="text-xl font-semibold text-gray-800 dark:text-white">{currentUser?.name || "please update name"}</h1>
  ...
  <path fill-rule="evenodd" d="M7 2a2 2 0 00-2 2v12a2 2 0 002 2h6a2 2 0 002-2V4a2 2 0 00-2-2H7zm3 14a1 1 0 100-2 1 1 0 000 2z" clip-rule="evenodd" />
                    ...
                    <h1 class="px-2 text-sm">{currentUser.contact}</h1>
                </div>
